...
// ===================================
// Original grid animation removed for Infinite Marquee CSS implementation
// The marquee uses CSS animations for smooth infinite scrolling and Opacity 1


// ===================================
// Robust Fallback for Content Loading
// ===================================
// Ensure all animated elements are visible even if JS fails or triggers don't fire quickly
window.addEventListener('load', () => {
  // Give GSAP time to process, then force opacity 1 on elements that might be stuck
  setTimeout(() => {
    gsap.to('.benefit-card, .service-card, .equipment-card, .doctor-grid, .about-grid', {
      opacity: 1,
      y: 0,
      clearProps: 'opacity,transform', // Clean up after overriding
      duration: 0.5,
      overwrite: 'auto'
    });
  }, 3000); // 3-second fail-safe
});

// ===================================
// Missing Animations for Services & Equipment
// ===================================
gsap.from('.service-card', {
  scrollTrigger: {
    trigger: '.services',
